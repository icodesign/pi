...
export interface Settings {
	...
	modelThinkingLevels?: Record<string, ThinkingLevel>; // per-model overrides keyed by "provider/modelId"
...
export interface SettingsStorage {
	...
	pathForScope?(scope: SettingsScope): string | undefined;
}
...
export class SettingsManager {
	...
	protected static loadState(
		...
		if (globalLoad.error) {
			initialErrors.push(toSettingsError("global", globalLoad.error, storage.pathForScope?.("global")));
		}
		if (projectLoad.error) {
			initialErrors.push(toSettingsError("project", projectLoad.error, storage.pathForScope?.("project")));
		}

		return {
			globalSettings: globalLoad.settings,
			projectSettings: projectLoad.settings,
			globalLoadError: globalLoad.error,
			projectLoadError: projectLoad.error,
			initialErrors,
			projectTrusted,
		};
	}
	...
	protected static migrateSettings(settings: Record<string, unknown>): Settings {
	...
	private recordError(scope: SettingsScope, error: unknown): void {
		this.errors.push(toSettingsError(scope, error, this.storage.pathForScope?.(scope)));
	}
...
/**
 * Structural settings owner used by the portable AgentSession core.
 * SettingsManager is the Node implementation, but callers may inject any
 * owner that implements the methods consumed by session and agent setup.
 */
export type SettingsManagerContract = Pick<
	SettingsManager,
	| "getBlockImages"
